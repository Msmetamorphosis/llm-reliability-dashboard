...
Endpoints:
  ...
  POST /api/run             - run experiment, stream results via SSE
  GET  /api/results/raw     - all stored experiment records (JSON or CSV)
  GET  /api/results/summary - OVS summary table by system x task
"""
...
async def startup():
    ...
    print(f"Loaded {len(task1_data)} Task1 / {len(task2_data)} Task2 / {len(corpus_data)} corpus")
...
def run_one_sample(
    ...
    # Existing scoring (preserved for dashboard compatibility)
    if task == "task1":
        ...
        acc = {
            "overall_accuracy": qa_score.get("overlap_score", 0.0),
            "fields_correct":   1 if qa_score.get("likely_correct", False) else 0,
            "fields_total":     1,
            "field_results":    {}
        }

    # UIBF metrics
    schema_pass  = 1 if result["schema_valid"] else 0
    parse_pass   = 1 if result["parseable"] else 0
    field_accuracy = (acc["overall_accuracy"] if task == "task1"
                      else qa_score.get("overlap_score", 0.0))
    structure_consistency = compute_structure_consistency(
        result["parsed_output"], schema, result["parseable"]
    )

    record = {
        # Existing fields (dashboard compatibility)
...
async def experiment_stream(req: RunRequest) -> AsyncGenerator[dict, None]:
    ...
    yield {"event": "start", "data": json.dumps({
        "total": total, "system": req.system,
        "task": req.task, "styles": styles,
        "model": req.model, "limit": req.limit
    })}
        ...
        for sample in samples:
            for style in styles:
                try:
                    ...
                    yield {"event": "error", "data": json.dumps({"message": "Invalid API key."})}
                    return
                except anthropic.RateLimitError:
                    yield {"event": "error", "data": json.dumps({"message": "Rate limit hit. Wait a moment."})}
                    return
                except Exception as e:
                    yield {
                        "event": "result",
...
async def results_raw(fmt: str = Query(default="json", description="json or csv")):
...
async def results_summary(fmt: str = Query(default="json", description="json or csv")):
    """Return OVS summary table by system x task."""
    records = load_results()
    if not records:
        return {"message": "No results stored yet. Run an experiment first.", "summary": []}
    summary = compute_ovs_summary(records)
    if fmt == "csv" and summary:
        ...
        return StreamingResponse(
            iter([output.getvalue()]),
            media_type="text/csv",
            headers={"Content-Disposition": "attachment; filename=ovs_summary.csv"}
        )
