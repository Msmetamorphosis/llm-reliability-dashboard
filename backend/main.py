...
Endpoints:
  ...
  GET  /api/datasets        - return dataset metadata
...
def run_one_sample(
    ...
    # Existing scoring (preserved for frontend compatibility)
    if task == "task1":
    ...
    # UIBF-aligned metrics
    schema_pass = 1 if result["schema_valid"] else 0
    parse_pass  = 1 if result["parseable"] else 0

    if task == "task1":
        field_accuracy = acc["overall_accuracy"]
    else:
        field_accuracy = qa_score.get("overlap_score", 0.0)
    ...
    record = {
        # Existing fields preserved for dashboard
        ...
        "craft_fidelity_score":  craft_fidelity_score,
    }
...
# ── UIBF export endpoints ─────────────────────────────────────────────────────

@app.get("/api/results/raw")
async def results_raw(fmt: str = Query(default="json", description="Response format: json or csv")):
...
async def results_summary(fmt: str = Query(default="json", description="Response format: json or csv")):
    """Return OVS summary table aggregated by system x task."""
    records = load_results()
    if not records:
